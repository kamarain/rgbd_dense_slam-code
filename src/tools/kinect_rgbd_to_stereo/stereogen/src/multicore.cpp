    ...
    for (blockID = 0; blockID < nBlocks; blockID++) {
        ...
        for (int j = blockID*blockSize; j < (blockID+1)*blockSize; j++) {
            for (int i = 0; i < width; i++,offset++) {
                ...
                fullPointSet[offset].rx      = p3[0];
                fullPointSet[offset].ry      = p3[1];
                fullPointSet[offset].rz      = p3[2];
...
void OMPFunctions::baselineWarpRGB(Mat &depthImageL,Mat &rgbImageR,float *KL, float *TLR, float *KR, float *kc, ProjectData *fullPointSet) {
    int width      = depthImageL.cols;
    int height     = depthImageL.rows;
    float *zptrSrc = (float*)depthImageL.ptr();
