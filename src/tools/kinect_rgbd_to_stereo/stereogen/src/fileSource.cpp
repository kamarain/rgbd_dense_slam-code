...
FileSource::FileSource(const char *filenameRGB, const char *filenameDepth) {
    fileRGB   = std::string(filenameRGB);
    fileDepth = std::string(filenameDepth);
    singleFileMode = true;
}
...
bool FileSource::loadSingleRGBD(Calibration *calib,int targetResoX, int targetResoY) {
    char buf[512];
    sprintf(buf,"%s",fileRGB.c_str());
    Mat rgbHeader = imread(buf,-1);
    bool loadOk = true;
    if (rgbHeader.data == NULL) { printf("file %s not valid!\n",buf); fflush(stdout); loadOk =false; }
    else {
        ...
        if (rgbHeader.cols == 320 && rgbHeader.rows == 240) {
            ...
            cvtColor(rgbHeader,rgbMat,CV_RGB2BGR);
...
//    prevLoadIndex = loadIndex;
    ...
    if (singleFileMode) {
        return loadSingleRGBD(calib,targetResoX,targetResoY);
    }
