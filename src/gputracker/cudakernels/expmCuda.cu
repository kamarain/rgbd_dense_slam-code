...
//F77 routines
///////////////////////////////////////////
__device__ int s_stop(char *s, ftnlen n) {
	//printf("%s\n",s);
	return 0; /* DUMMY */
}

__device__ void d_cnjg(doublecomplex *r, doublecomplex *z) {
	doublereal zi = z->i;
	r->r = z->r;
	r->i = -zi;
}

__device__ integer pow_ii(integer *ap, integer *bp)
{
	integer pow, x, n;
	unsigned long u;
...
__device__ void z_div(doublecomplex *c, doublecomplex *a, doublecomplex *b)
{
...
__device__ doublereal f__cabs(doublereal real, doublereal imag) 
{
	doublereal temp;
	if(real < 0)
		real = -real;
...
__device__ doublereal z_abs(doublecomplex *z) {
...
__device__ integer c__0 = 0;
__device__ integer c__1 = 1;
__device__ doublecomplex c_b113 = {-1.,-0.};

__device__ int zgefa_(doublecomplex *a, integer *lda, integer *n, integer *ipvt, integer *info)
{
...
__device__ int zgesl_(doublecomplex *a, integer *lda, integer *n, integer *ipvt, doublecomplex *b, integer *job)
{
	/* System generated locals */
	integer a_dim1, a_offset, i__1, i__2, i__3, i__4;
...
__device__ int zgesv_(integer *n, integer *m, doublecomplex *a, integer *lda, integer *ipiv, doublecomplex *b, integer *ldb, integer *iflag)
{
	/* System generated locals */
...
// dummy error handler
__device__ int xerbla2_(char *srname, integer *info, ftnlen srname_len)
{
	return 0;
} /* xerbla2_ */
...
/* ----------------------------------------------------------------------| */
__device__ long lsame_(char *ca, char *cb, ftnlen ca_len, ftnlen cb_len)
{
	/* System generated locals */
	long ret_val;
...
__device__ int zgemm_(char *transa, char *transb, integer *m, integer *
...
__device__ int zscal_(integer *n, doublecomplex *za, doublecomplex *zx, integer *incx)
{
	/* System generated locals */
...
__device__ doublereal dcabs1_(doublecomplex *z__)
{
	/* System generated locals */
	doublereal ret_val;
...
__device__ integer izamax_(integer *n, doublecomplex *zx, integer *incx)
{
	/* System generated locals */
...
__device__ int zaxpy_(integer *n, doublecomplex *za, doublecomplex *zx, integer *incx, doublecomplex *zy, integer *incy)
{
	/* System generated locals */
...
__device__ void zdotc_(doublecomplex * ret_val, integer *n, doublecomplex *zx, integer *incx, doublecomplex *zy, integer *incy)
{
	/* System generated locals */
	integer i__1, i__2;
	doublecomplex z__1, z__2, z__3;
...
__device__ int zdscal2_(integer *n, doublereal *da, doublecomplex *zx, integer *incx)
{
	/* System generated locals */
	integer i__1, i__2, i__3;
	doublecomplex z__1, z__2;
...
__device__ doublereal c_b3 = 1.;
__device__ doublereal c_b8 = 10.;
__device__ integer c__9 = 9;
__device__ integer c__3 = 3;
__device__ integer c__5 = 5;
__device__ integer c__6 = 6;
__device__ doublereal c_b65 = 0.;
__device__ integer c__2 = 2;
__device__ doublereal c_b127 = -1.;
__device__ doublereal c_b131 = 2.;
__device__ doublecomplex c_b172 = {0.,0.};
__device__ doublecomplex c_b173 = {1.,0.};

__device__ int zgpadm_(integer *ideg, integer *m, doublereal *t, 
...
__device__ void matrixMult4Real(doublereal *M1, doublereal *M2, doublereal *R)
{
        doublereal Rtmp[16];
...
/*
__device__ void normalizeMat4Real(doublereal *T) {
   doublereal len = 0.0f;
...
__device__ void normalizeVec3Real(doublereal *v) {
    doublereal len = sqrtf(v[0]*v[0]+v[1]*v[1]+v[2]*v[2]);
    v[0] /= len;  v[1] /= len; v[2] /= len;
}

__device__ void crossproductReal(doublereal *u, doublereal *v, doublereal *w) {
    w[0] = u[1]*v[2]-u[2]*v[1];
    w[1] = -(u[0]*v[2]-u[2]*v[0]);
    w[2] = u[0]*v[1]-u[1]*v[0];
}
...
__global__ void expmKernel(doublereal *mtx, float *TF, doublereal scale)
{
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx != 0) return;
    integer ideg = 6; // padé order number
    integer m = 4;
    integer M = 4;
    integer ldh = M;
    ...
    if (maxEntry != 0) {
        ...
        normalizeMat4Real(Tinc); Tinc[3] *= scale; Tinc[7] *= scale; Tinc[11] *= scale;
    ...
    matrixMult4Real(T,Tinc,T); normalizeMat4Real(T);
...
/*
extern "C" void expmCuda(double * mtx, double * output)
{
    assert(mtx != NULL && output != NULL);

	doublereal *mtxDev;
	doublereal *outputDev;

	cudaMalloc((void **)&mtxDev, 16*sizeof(doublereal));
	cudaMalloc((void **)&outputDev, 16*sizeof(doublereal));

	doublereal mtxHost[16];
	doublereal outputHost[16];

	for (int i = 0; i < 16; i++) mtxHost[i] = (doublereal)mtx[i];

	cudaMemcpy( mtxDev, mtxHost, 16*sizeof(doublereal), cudaMemcpyHostToDevice );
	
	expmKernel<<<1,1>>>(mtxDev,outputDev);
	cudaThreadSynchronize();
    getLastCudaError("expmKernel() execution failed\n");
    
	cudaMemcpy(outputHost, outputDev, sizeof( doublereal ) * 16, cudaMemcpyDeviceToHost );
	for (int i = 0; i < 16; i++) output[i] = (double)outputHost[i];
	cudaFree(outputDev);
	cudaFree(mtxDev);
}*/
...
extern "C" void expmCuda(doublereal * mtxDev, float * outputDev, float scaleOut)
{
    assert(mtxDev != NULL && outputDev != NULL);
    expmKernel<<<1,1>>>(mtxDev,outputDev,doublereal(scaleOut));
    getLastCudaError("expmKernel() execution failed\n");
}
