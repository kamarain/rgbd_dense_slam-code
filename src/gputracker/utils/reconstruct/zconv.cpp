...
void ZConv::increaseDynamics(float *dispImage,int w, int h, float scale) {
    int len = w*h;
   /* float *tmp = new float[len];
    int cnt = 0;
    for (int i = 0; i < len; i++) {
        if (dispImage[i] > 0) {
            tmp[cnt] = dispImage[i]; cnt++;
        }
    }
    float medianD = quickMedian(tmp,cnt);
    dumpDepthRange(tmp,cnt,1);
    delete[] tmp;
*/
    for (int i = 0; i < len; i++) {
        //float val = (dispImage[i]-medianD)*scale+dispImage[i];
        float val = dispImage[i]*scale;
        if (val < 0) val = 0.0f;
        dispImage[i] = val;
    }
}
