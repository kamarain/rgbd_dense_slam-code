...
#include <PoissonRecon/PoissonRecon-wrapper.h>
...
bool KeyFrameModel::generateTexture() {
    ...
    for (ki = keyFrames.begin(); ki != keyFrames.end(); ki++,index++) {
        ...
        int tileY = index/6;
        if (tileY > 7) {
            printf("invalid keyframe tile: (%d,%d) (48 is the limit!)\n",tileX,tileY); fflush(stdout);
            return false;
        }
...
bool KeyFrameModel::saveKeyImages() {
    char buf[512];
    if (!generateTexture()) return false;
...
void KeyFrameModel::savePolygonModel(char *filename, int numThreads, int depth, bool useClipBox, float *boxParams, VertexBuffer2 **vbuf, TriangleBuffer2 **tribuf) {
    // generate global cloud and clip it against given box:
    float *points = NULL;
    float *normals = NULL;
    ...
    float *vertices=NULL;   int totalVerts=0;
    unsigned int *faceIndex3=NULL; int nFaces3=0;
    if (generatePoissonMesh(numThreads,depth,points,normals,cnt,&vertices,&totalVerts,&faceIndex3,&nFaces3)) {
        char buf[512];
        sprintf(buf,"%s/%s",scratchDir,filename);
        writeWaveFront(buf, vertices, totalVerts, faceIndex3, nFaces3, &mtx4x4[0], tribuf);
        delete[] faceIndex3; delete[] vertices;
    } else {
        printf("poisson mesh generation failed: %s\n",filename);
    }
...
{
    if (nFaces <= 0 || keyFrames.size() < 1) {
        printf("nfaces: %d, nkeys: %d .. exiting generateUV3!\n",nFaces,keyFrames.size());
        return;
    }
    float *K    = &m_calib.getCalibData()[KR_OFFSET];
    float *kc   = &m_calib.getCalibData()[KcR_OFFSET];
    ...
    int width  = m_calib.getWidth();
    int height = m_calib.getHeight();
    printf("calibration: %d x %d\n",width,height); fflush(stdout);
    const int maxNeighbourCount = 128;
    float *faceArea      = new float[nFaces];
...
//                        faceArea[face] = i;
    ...
    reguralizeUVMapping(nFaces,&bestKey[0],faceLinks,maxNeighbourCount,&validUV[0],20,2);
...
void KeyFrameModel::writeWaveFront(const char *filename, float *vertices, int totalVerts, unsigned int *faceIndex3, int nFaces3, float *mtx4x4, TriangleBuffer2 **tribuf) {
    ...
    if (!saveKeyImages()) {
        printf("writeWaveFront: error in saving keyframe images!\n"); fflush(stdout);
        return;
    }
    ...
    printf("totalverts: %d, polygon count:%d \n",totalVerts,nFaces3); fflush(stdin); fflush(stdout);
    ...
    generateUV3(vertices,faceIndex3,nFaces3,f);
...
//            fprintf(f," %d",faceIndex3[i*3+j]+1);
        }
        fprintf(f,"\n");
    }
    fclose(f);
