...
void KeyFrame::selectPixels(Mat &img, Mat &depth, Mat &baseMask, Mat &pixelSelectionMask, int amount) {
...
void KeyFrame::selectPixels(Mat &img, Mat &depth, Mat &baseMask, Mat &pixelSelectionMask, float percent)
{
    unsigned char *selectionMask = pixelSelectionMask.ptr();
    memset(selectionMask,0,img.cols*img.rows);
    float *zptr = (float*)depth.ptr();
...
void KeyFrame::updateFrustum(float viewDistanceMin, float viewDistanceMax) {
    float sX,sY,len;
    float z[3],u[3],v[3];
    float o[3];
...
void KeyFrame::getScenePoint(int i, int j, Calibration &calib, float *x, float *y, float *z) {
...
void KeyFrame::getRay(int i, int j, int nX, int nY, RAY *ray)
{
    ray->o[0] = T[12];
    ray->o[1] = T[13];
    ray->o[2] = T[14];
...
void KeyFrame::setCalibDevPtr(float *calibDataDev, float normalizedK11, float normalizedK22) {
   ...
   // printf("keyframe fovX: %f, fovY: %f\n",fovAngleX,fovAngleY);
}


void KeyFrame::selectPixels(float pixelSelectionPercent) {
...
void KeyFrame::selectPixelsGPUCompressed(VertexBuffer2 &vbufferExt, int pixelSelectionAmount,Image2 *imDepth, bool rgbVisualization) {
    vbuffer.lock();
    vbuffer.lockIndex();
...
//    cudaHista(gradientScratchDev, histogramFloatDev, vbufferExt.getVertexCount(), 256,d_hist);
    ...
    histogram256VBufGradMag(histogramDev,gradientScratchDev,vbufferExt.getVertexCount(),pixelSelectionAmount);
...
void KeyFrame::selectPixelsGPUCompressedLock(VertexBuffer2 &vbufferExt, int pixelSelectionAmount,Image2 *imDepth, ImagePyramid2 *grayPyramidExt) {
    vbuffer.lock();
    vbuffer.lockIndex();
    extractGradientMagnitudes(&vbufferExt,gradientScratchDev);
    //cudaHista(gradientScratchDev, histogramFloatDev, vbufferExt.getVertexCount(), 256,d_hist);
...
/*
void KeyFrame::selectPixelsGPUCompressed2(float *verticesExt, int *indicesExt, int vertexCount, int pixelSelectionAmount, int stride) {
...
void KeyFrame::normalizeDepthMap(cv::Mat &depthMap, cv::Mat &normalizedMap, float depthMin, float depthMax) {
...
void KeyFrame::generatePointsWithoutNormals(Calibration *calib) {
...
void KeyFrame::generatePoints(Calibration *calib)
{
    int fw = depthCPU.cols;
    int fh = depthCPU.rows;
...
void KeyFrame::generateGPUData(int pixelSelectionAmount, bool renderableVBuffer, bool optimizeGPUMemory, bool undistortRGB, bool rgbVisualization,Calibration *calib, unsigned char *imRGBDev, float *imDepthDev, Image2 &depth1C, VertexBuffer2 *vbufferTemp, int nMultiResolutionLayers)
{
    ...
    cvtColor(distortedRgbImageCPU,rgbImage2,CV_RGB2BGR);
...
void KeyFrame::setBaseTransform(float *baseDev) {
...
void KeyFrame::setupCPUTransform() {
    float mInv[16];
    float mDev[16];
...
void KeyFrame::resetRelativeDev() {
    cudaMemcpyAsync((void*)TrelDev, (void*)TidentityDev,16*sizeof(float),cudaMemcpyDeviceToDevice,0);
}

// inputT in OpenGL compatible form already
void KeyFrame::updateTransformGL(float *inputT) {
...
void KeyFrame::updateTransform(float *inputT) {
    transpose4x4(inputT,&T[0]);
    memcpy(&Tbase[0],&T[0],sizeof(float)*16);
    setupCPUTransform();
}

void KeyFrame::setupRelativeCPUTransform(float *T) {
    float relativeT[16];
    float invCur[16];
...
void KeyFrame::setupRelativeGPUTransform(float *TDev) {
    invertPoseCuda(TabsDev, TtmpDev, 1);
    matrixMult4Cuda(TDev,TtmpDev,TrelDev);
}

void KeyFrame::getRelativeTransform(float *T, float *relativeT) {
    float invCur[16];
    float mRef[16],mCur[16];
    transpose4x4(&Tbase[0],&mRef[0]);
    transpose4x4(&T[0],&mCur[0]);
...
KeyFrame::KeyFrame(int width, int height) {
    visible = true;
    ...
    if (keyFrameRefCount == 0) {
        initCudaDotProduct(maxResidualSize);
        ...
        cudaMalloc((void **)&histogramDev, 256 * sizeof(unsigned int));
        ...
        adaptedInitHistogram64();
        adaptedInitHistogram256();
...
void KeyFrame::reallocJacobian(int pixelSelectionAmount) {
...
void KeyFrame::allocateJacobian(int pixelSelectionAmount) {
...
void KeyFrame::setIterationCounts(int *nIter) {
...
void KeyFrame::optimizePose(ImagePyramid2 &grayPyramid, VertexBuffer2 *vbufferCur, bool filterDepthFlag)
{
...
void KeyFrame::updateBase(BaseBuffer2 &baseBuffer, ImagePyramid2 &grayPyramid) {
...
void KeyFrame::release() {
    ...
    if (keyFrameRefCount == 0) {
        ...
        adaptedCloseHistogram64();
        adaptedCloseHistogram256();
...
/*
void KeyFrame::initializeFeatures(int nFeatures, int keyIndex, Calibration *calib) {
...
/*
void KeyFrame::searchFeatures(KeyFrame *curFrame, int keyIndex, Calibration *calib, int searchRegion, float depthTolerance) {
...
void getSearchBound(KeyFrame *key0, KeyFrame *key1, float x, float y, float searchBoundZ, Calibration &calib, float *p1, float *p2) {
    float T0[16],T1[16];
...
void getSearchBound(KeyFrame *key0, KeyFrame *key1, float *p3d, float searchBoundZ, Calibration &calib, float *p1, float *p2) {
    float T0[16],T1[16];
    transpose4x4(&key0->T[0],&T0[0]);
    transpose4x4(&key1->T[0],&T1[0]);
...
void getProjection(KeyFrame *key0, KeyFrame *key1, float *p3d, Calibration &calib, float *p ) {
    float T0[16],T1[16];
    transpose4x4(&key0->T[0],&T0[0]);
    transpose4x4(&key1->T[0],&T1[0]);
...
void KeyFrame::gpuCopyKeyframe(int id, ImagePyramid2 &frame1C, Image2 &frame3C, VertexBuffer2 *vbufferExt, float *imDepthDevIR, int pixelSelectionAmount, VertexBuffer2 *vbufferTemp, int nMultiResolutionLayers, bool renderableVBuffer) {
