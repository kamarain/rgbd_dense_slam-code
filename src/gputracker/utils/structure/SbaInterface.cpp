...
void sba_driver(SparseBundleConfiguration &sbaConf, char *camsfname, char *ptsfname, char *calibfname, int cnp, int pnp, int mnp,
                ...
                int filecnp, char *refcamsfname, char *refptsfname)
{
  ...
  register int i;
  FILE *fp;
  double *camParams = sbaConf.getCameraParams();
  double *pointParamsSave;
  int numCurrentSavedPoints = 0;
  sbaConf.getPointsSave(&pointParamsSave,&numCurrentSavedPoints);
  ...
  // store tuned camera params
  for (int i = 0; i < nframes; i++) {
      camoutfilter(motstruct+i*cnp, cnp, &camParams[i*filecnp], filecnp);
  }
  // store tuned 3d points
  for (int i = 0; i < numpts3D; i++) {
      pointParamsSave[i*3+0] = motstruct[nframes*cnp+i*3+0];
      pointParamsSave[i*3+1] = motstruct[nframes*cnp+i*3+1];
      pointParamsSave[i*3+2] = -motstruct[nframes*cnp+i*3+2];
  }
 // printSBAData(stdout, motstruct, cnp, pnp, mnp, camoutfilter, filecnp, nframes, numpts3D, imgpts, numprojs, vmask);
//  printSBAMotionData(stdout, motstruct, nframes, cnp, camoutfilter, filecnp);
...
void SbaInterface::exec(SparseBundleConfiguration &sbaConf) {
    sbaConf.saveArguments();
    int cnp=6, // 3 rot params + 3 trans params
        pnp=3, // euclidean 3D points
        mnp=2; // image points are 2D
    sba_driver(sbaConf,sbaConf.getCamsFileNameArg(),sbaConf.getPointsFileNameArg(),sbaConf.getCalibFileNameArg(),cnp,pnp,mnp,quat2vec,vec2quat,cnp+1,"-","-");
    sbaConf.updateCameras();
}
