...
double *tofilter;
  ...
  while(!feof(fp)){
    ...
    if(infilter){
      ...
      //tofilter[3] = -tofilter[3];
      //tofilter[4] = -tofilter[4];
...
register int i, ii, jj, k;
  ...
  while(!feof(fp)){
    ...
    // convert input points from right to left! tommi tykkala 8.9.2012 (check also camera params and intrinsic matrix!)
     params[2] = -params[2];
...
void readCalibParams(char *fname, double ical[9])
{
  ...
  fclose(fp);
  // convert intrisic matrix to correspond left handed system! tommi tykkala 8.9.2012 (check also camera params and point params!)
  K[0] = fabs(K[0]);
  K[4] = -fabs(K[4]);
  //printf("sba K:\n");
