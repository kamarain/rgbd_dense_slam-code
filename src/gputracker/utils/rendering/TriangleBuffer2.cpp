...
//    devPtr = NULL;
    ...
    this->maxNumElements = numTriangles*3;
     ...
     m_vertexBuffer.alloc(numElements*stride, true, false);    // create as VBO
//     m_vertexBuffer2.alloc(numElements*stride, true, false);    // create as VBO
     printf("numElements: %d\n",numElements); fflush(stdout);
//     printFreeDeviceMemory();
     printf("numElements*stride: %d, stride:%d, numElems:%d\n",numElements*stride,stride,numElements); fflush(stdout);


    m_indexBuffer.alloc(numElements*stride, true, false); // create as index buffer
    this->cudaStream     = 0;
...
void *TriangleBuffer2::lock() {
    m_vertexBuffer.map();
    return m_vertexBuffer.getDevicePtr();
}
...
void *TriangleBuffer2::lockIndex() {
    m_indexBuffer.map();
    return m_indexBuffer.getDevicePtr();
}
...
void TriangleBuffer2::render() {
    ...
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, m_indexBuffer.getVbo());
