...
void ImagePyramid2::gaussianBlur( Image2 *img, int size )
{
	...
	CvMat *oldmatA = cvCreateMat(img->height,img->width,CV_8U);
	CvMat *oldmatB = cvCreateMat(img->height,img->width,CV_8U);	
	memcpy(oldmatA->data.ptr,img->data,img->pitch*img->height);
	cvSmooth(oldmatA,oldmatB,CV_GAUSSIAN,size);
	memcpy(img->data,oldmatB->data.ptr,img->pitch*img->height);
...
void ImagePyramid2::updateLayers()
{
	if (pyramid[0].onlyGPUFlag) {
		downSample2Pyramid(*this);
/*		for (int i = 1; i < nLayers; i++) {
